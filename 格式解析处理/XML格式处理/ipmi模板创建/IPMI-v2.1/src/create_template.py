...
#                 threshold_description = application_chinese[0]+'传感器ID: {#SENSOR_ID}\n' + application_chinese[0] +'传感器名字: {#SENSOR_NAME}\n' + application_chinese[0]+'传感器阈值\n'+'严重告警: {#THRESH_HIGH_WARN} , {#THRESH_HIGH_AVG} , {#THRESH_HIGH_CRIT} \n轻微告警: {#THRESH_LOW_WARN} , {#THRESH_LOW_AVG} , {#THRESH_LOW_CRIT}'
            ...
            preprocessing.appendChild(dom.createElement('params')).appendChild(dom.createTextNode("$.[?(@.id=='{#SENSOR_ID}')].first()")) 
            if application_chinese:
                ...
                item_prototype.appendChild(dom.createElement('name')).appendChild(dom.createTextNode('传感器: {#SENSOR_ID}的当前序列内容'))
...
#             preprocessing.appendChild(dom.createElement('params')).appendChild(dom.createTextNode("$.[?(@.id=='{#SENSOR_ID}')].state.state.first()"))        
            ...
            item_prototype.appendChild(dom.createElement('key')).appendChild(dom.createTextNode('ipmi.sensor.'+ application +'[{#SENSOR_NAME}]'))
            ...
            item_prototype.appendChild(dom.createElement('value_type')).appendChild(dom.createTextNode('TEXT'))
...
if __name__ == '__main__':
    json_file = './host_json/10593_192.168.50.11.json'
